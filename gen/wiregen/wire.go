...
// Package wiregen provides compile-time dependency injection logic as a
// Go library.
package wiregen
...
func Gen(files []*dst.File, filenames []string, pkg *decorator.Package) (err error) {
	oc := newObjectCache([]*packages.Package{pkg.Package})
	for _, f := range files {
		for _, decl := range f.Decls {
			...
			fn2 := pkg.Decorator.Ast.Nodes[fn].(*ast.FuncDecl)
			buildCall, err := findInjectorBuild(pkg.Package.TypesInfo, fn2)
			if err != nil {
				continue
			}
			if buildCall == nil {
			...
			sig := pkg.TypesInfo.ObjectOf(fn2.Name).Type().(*types.Signature)
			...
			g := newGen(pkg.Package)
			if errs := g.inject(fn2.Pos(), fn.Name.Name, sig, set, fn2.Doc); len(errs) > 0 {
				continue
			}
			f2, err := pkg.Decorator.ParseFile("", "package main\n"+g.buf.String(), 0)
			if err != nil {
				return err
			}
			fn.Body = f2.Decls[0].(*dst.FuncDecl).Body
		}
	}
	return nil
}
