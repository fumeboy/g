...
func (p *providerSetSrc) description(fset *token.FileSet, typ types.Type) string {
	...
	case p.Binding != nil:
		return fmt.Sprintf("wiregen.Bind (%s)", fset.Position(p.Binding.Pos))
	case p.Value != nil:
		return fmt.Sprintf("wiregen.Value (%s)", fset.Position(p.Value.Pos))
	case p.Import != nil:
	...
	case p.Field != nil:
		return fmt.Sprintf("wiregen.FieldsOf (%s)", fset.Position(p.Field.Pos))
	}
...
type ProviderSet struct {
	// Pos is the position of the call to wiregen.NewSet or wiregen.Build that
	// created the set.
	Pos token.Pos
	...
	Fields    []*Field
	Imports   []*ProviderSet
	// InjectorArgs is only filled in for wiregen.Build.
...
type Value struct {
	...
	// expr is the expression passed to wiregen.Value.
	expr ast.Expr
...
func (oc *objectCache) processNewSet(info *types.Info, pkgPath string, call *ast.CallExpr, args *InjectorArgs, varName string) (*ProviderSet, []error) {
	// Assumes that call.Fun is wiregen.NewSet or wiregen.Build.
...
// wiregen.Struct syntax for those.
func processStructLiteralProvider(fset *token.FileSet, typeName *types.TypeName) (*Provider, []error) {
	...
	fmt.Fprintf(os.Stderr,
		"Warning: %v, see https://godoc.org/github.com/google/wiregen#Struct for more information.\n",
		notePosition(fset.Position(pos),
			fmt.Errorf("using struct literal to inject %s is deprecated and will be removed in the next release; use wiregen.Struct instead",
				typeName.Type())))
	provider := &Provider{
		Pkg:      typeName.Pkg(),
		Name:     typeName.Name(),
...
func processStructProvider(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Provider, error) {
	// Assumes that call.Fun is wiregen.Struct.
...
// Since this is the only tag used by wiregen, we can do string comparison
// without using reflect.
func isPrevented(tag string) bool {
	return reflect.StructTag(tag).Get("wiregen") == "-"
}

// processBind creates an interface binding from a wiregen.Bind call.
func processBind(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*IfaceBinding, error) {
	// Assumes that call.Fun is wiregen.Bind.
...
// processValue creates a value from a wiregen.Value call.
func processValue(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Value, error) {
	// Assumes that call.Fun is wiregen.Value.
	...
	// Result type can't be an interface type; use wiregen.InterfaceValue for that.
...
// processInterfaceValue creates a value from a wiregen.InterfaceValue call.
func processInterfaceValue(fset *token.FileSet, info *types.Info, call *ast.CallExpr) (*Value, error) {
	// Assumes that call.Fun is wiregen.InterfaceValue.
...
// processFieldsOf creates a slice of fields from a wiregen.FieldsOf call.
func processFieldsOf(fset *token.FileSet, info *types.Info, call *ast.CallExpr) ([]*Field, error) {
	// Assumes that call.Fun is wiregen.FieldsOf.
	...
	for i := 1; i < len(call.Args); i++ {
		...
		if isPtrToStruct {
			...
			// wiregen.Fields also provides a pointer to the field.
...
func checkField(f ast.Expr, st *types.Struct) (*types.Var, error) {
	...
	for i := 0; i < st.NumFields(); i++ {
		if strings.EqualFold(strconv.Quote(st.Field(i).Name()), b.Value) {
			if isPrevented(st.Tag(i)) {
				return nil, fmt.Errorf("%s is prevented from injecting by wiregen", b.Value)
			}
...
// findInjectorBuild returns the wiregen.Build call if fn is an injector template.
...
func findInjectorBuild(info *types.Info, fn *ast.FuncDecl) (*ast.CallExpr, error) {
	...
	if invalid {
		return nil, errors.New("a call to wiregen.Build indicates that this function is an injector, but injectors must consist of only the wiregen.Build call and an optional return")
	}
...
// can be a *Provider (a provider function), a *Value (wiregen.Value), or an
...
// bindShouldUsePointer loads the wiregen package the user is importing from their
// injector. The call is a wiregen marker function call.
func bindShouldUsePointer(info *types.Info, call *ast.CallExpr) bool {
	// These type assertions should not fail, otherwise panic.
	fun := call.Fun.(*ast.SelectorExpr)                 // wiregen.Bind
	pkgName := fun.X.(*ast.Ident)                       // wiregen
	wireName := info.ObjectOf(pkgName).(*types.PkgName) // wiregen package
